...
/*  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 	
	...
	This script handle student assignment questions
	~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
session_id();
...
session_start();
     ...
     define('wizGrade', 'igweze');  /* define a check for wrong access of file */
        ...
        require 'configwizGrade.php';  /* load wizGrade configuration files */
			...
			if ($_REQUEST['assignQuestionData'] == 'updateAssignQuestion') {  /* save assignment question */ 
				...
				if ($qID == ""){
					...
					echo "<script type='text/javascript'>   $('#editLoader').fadeOut(1500); </script>";exit;
         			...
         			echo "<img src=''   height = '1' width='1'> ";
					$msg_e = "* Oooooooops Error, please select an Assignment to add Question";
					...
					echo "<img src=''   height = '1' width='1'> ";
					$msg_e = "* Oooooooops Error, please enter Assignment Question";
					echo $errorMsg.$msg_e.$eEnd; 
					echo "<script type='text/javascript'>   $('#editLoader').fadeOut(1500); </script>";exit;
					...
					if(strlen($name)) {
								...
								if (move_uploaded_file($_FILES[$filePic]['tmp_name'], $picturePath.$uploadedPic )) { /* move uploaded file to its path */
									...
									try { 
										...
										if($qID >= $fiVal){
											...
											if($qPic != ""){
												...
												if(file_exists($oldQpic)){
													...
													unlink($oldQpic);  /* remove picture */ 	
													
												}	

											}	
									
											$ebele_mark = "UPDATE $wizGradeAssignQuestionTB  
											...
											$ebele_mark = "INSERT INTO $wizGradeAssignQuestionTB  (eID, question, qPicture, qOptions, qAnswer, qMark)
										...
										if($igweze_prep->execute()){ /* insert picture name to database */
											...
											echo "<img src=''   height = '1' width='1'> ";
											$msg_s = "Assignment Question was successfully saved."; 
											echo $succesMsg.$msg_s.$sEnd; 
											echo "<script type='text/javascript'> $('#assignQuesDiv').load('wizGradeAssignQuestionsInfo.php?eID=".$eID."'); 
											   $('#editLoader').fadeOut(1500);  $('#editAssignQuestionDiv').slideUp(1500);
											</script>";exit;
											
										}else{ /* display error messages */
											
											echo "<img src=''   height = '1' width='1'> ";
											$msg_e =  "Ooooooooops, an error has occur while to save Assignment Question. Please try again";
											echo $errorMsg.$msg_e.$eEnd; 
											echo "<script type='text/javascript'>   $('#editLoader').fadeOut(1500); </script>";exit;
						...
						try{	
							...
							if($qID >= $fiVal){
								...
								$ebele_mark = "INSERT INTO $wizGradeAssignQuestionTB  (eID, question, qOptions, qAnswer, qMark)
			...
			}elseif ($_REQUEST['assignQuestionData'] == 'removeAssignQuestion') {  /* remove assignment question */ 

				
				$assignQuestionData = $_REQUEST['rData'];
				
				list($wizGradeIg, $qID, $hName) = explode("-", $assignQuestionData);			
				
				/* script validation */ 
				
				if (($assignQuestionData == "")  || ($qID == "")){
			...
			}elseif ($_REQUEST['assignQuestionData'] == 'viewAssignQuestion') {  /* view assignment question */ 
				...
				if ($qID == ""){
					...
					echo "<script type='text/javascript'>   $('#editLoader').fadeOut(1500); </script>";exit;
		 			...
		 			try {
									...
									$assignQuestionInfoArr = assignQuestionInfo($conn, $qID);  /* online assignment question information */
									//$qID = $assignQuestionInfoArr[$fiVal]["qID"];
									$eID = $assignQuestionInfoArr[$fiVal]["eID"];
									$question = htmlspecialchars_decode($assignQuestionInfoArr[$fiVal]["question"]);
									$qPicture = $assignQuestionInfoArr[$fiVal]["qPicture"];
									$qOptions = htmlspecialchars_decode($assignQuestionInfoArr[$fiVal]["qOptions"]);
									$qAnswer = htmlspecialchars_decode($assignQuestionInfoArr[$fiVal]["qAnswer"]);
									$qMark = $assignQuestionInfoArr[$fiVal]["qMark"];
...
$showPayment =<<<IGWEZE
							...
							<button  class="btn btn-white printerIcon pull-right">
							...
							<table width = '100%' class="table table-striped  table-advance table-hover"> 
								...
								<tr><th style="padding-left: 30px; text-align:left; width: 30%;">
								Assignment Question </td> <td style="padding-left: 30px; 
								text-align:left; width: 70%;">
								$question
								</td> </tr>
...
IGWEZE;
			...
			}elseif ($_REQUEST['assignQuestionData'] == 'editAssignQuestion') {  /* edit assignment question */ 

				
				$qID = preg_replace("/[^0-9-]/", "", $_REQUEST['questionID']);
				$eID = preg_replace("/[^0-9-]/", "", $_REQUEST['assignID']);
				
				/* script validation */ 
				
				if ($eID == ""){
         			
					$msg_e = "* Ooooooooops, an error has occur while to retrieve Assignment Question information. Please try again";
					echo $errorMsg.$msg_e.$eEnd; 
					echo "<script type='text/javascript'>   $('#editLoader').fadeOut(1500); </script>";exit;
		 			...
		 			try {
							...
							if($qID >= $fiVal){  /* check if question ID is true */
								...
								$question = htmlspecialchars_decode($assignQuestionInfoArr[$fiVal]["question"]);
								$qPicture = $assignQuestionInfoArr[$fiVal]["qPicture"];
								$qOptions = $assignQuestionInfoArr[$fiVal]["qOptions"];
								$qMark = $assignQuestionInfoArr[$fiVal]["qMark"];
								$qAnswer = $assignQuestionInfoArr[$fiVal]["qAnswer"];
								$eQPic = $wizGradeQuestionDir.$qPicture;
...
?>
							...
							<form class="form-horizontal" id="frmupdateAssignQuestion" role="form" enctype="multipart/form-data" 
							action="wizGradeAssignQuestions.php"> 
									  
									<div class="form-group">
                                      <label for="question" class="col-lg-4 col-sm-4 control-label"> * Assignment Question</label>                                      
                                      <div class="col-lg-8">
                                        
                                            <textarea rows="4" cols="10" class="form-control" name="question" id="question"  required
                                            placeholder="Enter Assignment Question, can be indent as liked "><?php echo $question; ?></textarea>
                                          ...
                                          <div class="col-md-8">
                                              <div class="fileupload fileupload-new" data-provides="fileupload">
                                                  ...
                                                  <div>
                                                   ...
                                                   Upload File</span>
                                                   <input type="file" id="aQuestionPic" 
                                    ...
                                    <div class="form-group">
                                      	  <input type="hidden" name="assignQuestionData" value="updateAssignQuestion" />
										  <input type="hidden" name="qID" value="<?php echo $qID; ?>" />
										  <input type="hidden" name="eID" value="<?php echo $eID; ?>" />
										  <input type="hidden" name="qPic" value="<?php echo $qPicture; ?>" />										  
                                          <center><button type="submit" class="btn btn-danger buttonMargin" id="updateAssignQuestion">
