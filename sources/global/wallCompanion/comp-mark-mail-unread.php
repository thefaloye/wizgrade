				...
				require ($wizGradevalidater); 
		 		...
		 		try {
							...
							$wallPic, $load_page) = explode ("##", $memberInfo);	
							    ...
							    $mailStarIcon = '<i class="fa fa-star inbox-started"></i>';
								...
								foreach($_REQUEST as $mailMsg => $msg_id) {	 /* mark mail unread */					
							
									$igweze_prep->bindValue(':msg_id', $msg_id);																						
									$igweze_prep->execute();
																		
									echo  "<script type='text/javascript'>$('#mailRowID-$msg_id').addClass('unread'); 
