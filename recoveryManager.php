...
session_start();
		...
		if ($_REQUEST['resetData'] == 'to-nkiru-my-wife') {  /* check and send admin reset password link */ 
			...
			if (($adminMail == '') || (!filter_var($adminMail, FILTER_VALIDATE_EMAIL))){
				...
				$msg_e = "Oooooooops Error, please enter a valid email address";
				 ...
				 $msg_e = "Oooooooops Error, but we could not locate any admin account with <b>$adminMail</b>.Please check the spelling and try again";
				...
				if ($igweze_prep->execute()){
					...
					wizGradeMailer($adminMail, $subject, $message, "no-reply@wizgrade.com");

					$msg_s = "A link have been sent to your email address <b>($adminMail)</b>. Please check your inbox or spam folder for the mail and click on the link to reset your password";
					
					echo "<script type='text/javascript'>  $('.login-wrap-reset').slideUp(); $('#small_rloader').fadeOut(1500); </script>";  
					...
					echo $succesMsg.$msg_s.$sEnd; echo $scrollUp;  exit;
					 ...
					 $msg_e = "Ooooooooooops, an error has occur while trying to reset your password.
 			...
 			try {
				...
				if ($new_pass == '') {
					...
					if ($igweze_prep->execute()) {  /* if sucessfully */ 
						...
						$msg_s = "Your password was sucessfully updated. Always remember keep your password secret only to yourself."; 						
						...
						$msg_e = "*Ooooooooooops error, your password was not reset, please try again";
		...
		if ($msg_s) {
			...
			echo "<script type='text/javascript'> $('#small_rloader').fadeOut(1500); </script>";
			echo $succesMsg.$msg_s.$sEnd ; echo $scrollUp;  exit;
